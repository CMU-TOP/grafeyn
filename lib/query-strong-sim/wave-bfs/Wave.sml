...
struct
  ...
  type wave = (BasisIdx.t * Complex.t) option Seq.t
  ...
  fun size wave =
    SeqBasis.reduce 2000 op+ (0, Seq.length wave) (fn i =>
  ...
  fun advanceAndSplit {constraint, gate} wave =
    let
      fun upCapacity cap =
        Int.min (constraint, Real.ceil (1.5 * Real.fromInt capacity))

      fun upPrefixSize prefisSize =
        Int.min (2 * prefixSize, Seq.length wave)

      fun loop previousResult prefixSize capacity =
        let
          val _ = print
            ("advanceAndSplit |wave|=" ^ Int.toString (Seq.length wave)
             ^ " prefixSize=" ^ Int.toString prefixSize ^ " capacity="
             ^ Int.toString capacity ^ "\n")

          val prefix = Seq.take wave prefixSize
          val table = makeTable capacity
        ...
        handle HT.Full =>
          if capacity >= constraint then previousResult
      ...
      val startingCapacity = Int.min (capacity, Real.ceil
